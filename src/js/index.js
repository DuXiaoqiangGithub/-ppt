...
(function () {
    var $wrapper = $('.slider-wrapper'),
        ...
        slider_otem_len = $slider_item.length,
        active_num = 0,
        prev_num,
        radius_html = '',
        key = true,
        ...
        slider_change: function (direction) {
            if (key) {
                ...
                if (direction == 'prev' || 'next') {
                    if (direction == 'next') {
                        active_num = active_num + 1 > slider_otem_len - 1 ? 0 : active_num + 1;
                    } else {
                        active_num = active_num - 1 < 0 ? slider_otem_len - 1 : active_num - 1;
                    }
                } else {
                ...
                $slider_prev.animate({ opacity: 0},function () {
                    $slider_item.eq(active_num).css('display', 'block').animate({ opacity: 1 },
                    $.description_show)
                }).css('display', 'none');
